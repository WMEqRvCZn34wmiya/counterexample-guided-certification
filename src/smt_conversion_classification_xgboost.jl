"""
    to_smtlib2_classification_xgboost(expr; output_file = "output.smt2")

Extended to handle forest LRA encoding with proper type declarations.
"""
function to_smtlib2_classification_xgboost(
    expr;
    ...
    weight_vars = Set{String}()     # w_i_j variables

    collect_symbols_classification_xgboost(
        expr,
        vars,
        outputs,
        indicator_vars,
    ...
    smt_code = generate_smtlib2_classification_xgboost(
        expr,
        vars,
        outputs,
        indicator_vars,
...
"""
    collect_symbols_classification_xgboost(expr, vars, outputs, indicator_vars, count_vars, event_vars, z_var, weight_vars)

Extended to collect all variable types from forest encoding.
"""
function collect_symbols_classification_xgboost(
    expr,
    vars,
    outputs,
    indicator_vars,
    ...
    # Indicator variables w_i_j (Int/Boolean represented as 0/1)
    for m in eachmatch(r"\[w(\d+)_(\d+)\]", str)
        push!(weight_vars, "w$(m.captures[1])_$(m.captures[2])")
    end
...
"""
    generate_smtlib2_classification_xgboost(expr, vars, outputs, indicator_vars, count_vars, event_vars, z_var, weight_vars)

Extended to declare all variable types correctly.
"""
function generate_smtlib2_classification_xgboost(
    expr,
    vars,
    outputs,
    indicator_vars,
    ...
    if !isempty(indicator_vars)
        println(
            io,
            "; Indicator variables o_i_j (Int as Boolean 0/1) and w_i_j (Weight are Real)",
        )
        for ov in sort(
        ...
        for o in wv
            wvv = replace(o, "o" => "w")
            println(io, "(declare-const $wvv Real)")
        end
        println(io)
    end

    # Count variables c_j (Real) (was Int before 4th may 2026)
    if !isempty(count_vars)
        println(io, "; Count variables c_j (Real)")
        for cv in sort(
            collect(count_vars); by=x -> parse(Int, match(r"\d+", x).match)
        )
            println(io, "(declare-const $cv Real)")
        end
        println(io)
    end
    ...
    println(io, "(assert")
    convert_expr_classification_xgboost(io, expr, 1)
    println(io, ")")
    println(io)
...
"""
    convert_expr_classification_xgboost(io, expr, indent)

Extended to handle negation (¬) operator.
"""
function convert_expr_classification_xgboost(io, expr, indent)
    ind = "  "^indent
    ...
    if hasproperty(expr, :token)
        ...
        if token == "∧"
            println(io, ind * "(and")
            for child in expr.children
                convert_expr_classification_xgboost(io, child, indent + 1)
            end
            print(io, ind * ")")
        ...
        elseif token == "¬"
            ...
            converted = convert_condition_classification_xgboost(string(expr))
            print(io, ind * converted)
        end
    else
        converted = convert_condition_classification_xgboost(string(expr))
        converted = replace(converted, r"Atom\{.*\}\: " => "")
        converted = convert_condition_classification_xgboost(converted)
        print(io, ind * converted)
    end
...
"""
    convert_condition_classification_xgboost(str)

Extended to handle all variable patterns in forest encoding.
"""
function convert_condition_classification_xgboost(str)
    str = strip(str)
    ...
    # Pattern: [w_i_j] indicator variables
    for (op_pattern, op_smtlib) in [
        (">", ">"),
        ("<", "<"),
        (">=", ">="),
        ...
        m = match(
            Regex("\\[(w\\d+_\\d+)\\]\\s*$(op_pattern)\\s*([\\d.e\\-+]+)"), str
        )
        if m !== nothing
            return "($(op_smtlib) $(m.captures[1]) $(format_float(m.captures[2])))"
        end
...
"""
    parse_z3_model_classification_xgboost(model_str::String)

Parse Z3 model output into separate variable assignments.
"""
function parse_z3_model_classification_xgboost(model_str::String)
